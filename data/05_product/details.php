...
//读取笔记本商品的信息
$sql = "SELECT * FROM kj_laptop WHERE lid=$lid";
...
//print_r($output);
/*//读取笔记本商品的图片列表
$sql = "SELECT * FROM kj_laptop_pic WHERE laptop_id=$lid ORDER BY pid";
$result = mysqli_query($conn, $sql);
...
//print_r($fid);
$sql = "SELECT * FROM kj_laptop_family WHERE fid=$fid";
$result = mysqli_query($conn, $sql);
$output['family'] = mysqli_fetch_assoc($result);

//读取同型号的其它笔记本规格
$sql = "SELECT lid FROM kj_laptop WHERE family_id=$fid";
$result = mysqli_query($conn, $sql);
